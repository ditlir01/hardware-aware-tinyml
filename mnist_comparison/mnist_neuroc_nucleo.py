# Neuro-C MNIST sized for NUCLEO-F072RB deployment (128 KB Flash budget).
#
# This is the paper-faithful deployment target: same Cortex-M0 / 128 KB
# Flash / 16 KB RAM as the paper's STM32F072RB. The paper deploys
# Neuro-C models at ~20 KB total. We aim for the same envelope.
#
# Architecture: 784 -> 64 -> 32 -> 10  (2 hidden Neuro-C layers).
# With block-based encoding the deployed footprint is ~40 KB, fitting
# the NUCLEO's 128 KB flash with substantial headroom.
#
# A shallower / narrower model than the MSP432 variant — necessary
# because (a) the NUCLEO's flash is half the MSP432's, and (b) we want
# to demonstrate the architecture in roughly the paper's deployment
# envelope.
...
# NUCLEO-sized architecture: aggressively shallow + narrow to hit the
# paper's ~20 KB envelope on Cortex-M0. Expected accuracy ~93-95%.
...
model.save("mnist_neuroc_nucleo.keras")
print("Saved -> mnist_neuroc_nucleo.keras")
